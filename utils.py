...
_META_ARTIFACT_INTRO_ONLY = re.compile(
    r"^[^<\n]*?(?:таким образом|отметим|заметим|в данном обзоре|в данной статье|ниже представлены|ниже приведены|давайте рассмотрим|во-первых|во-вторых|в-третьих|среди прочего|в первую очередь|короче говоря|подытоживая|в частности|перейдём к|перейдем к|возвращаясь к|как показано|как мы видим|как можно заметить|рассмотрим|остановимся на|исходя из вышесказанного|исходя из этого|нужно отметить|необходимо отметить|хочется отметить|хочется подчеркнуть|важно подчеркнуть|не стоит забывать|особо следует|отдельно стоит|в этой связи|подведём итог|подведем итоги|по итогам)[^\n]*\n*",
    re.IGNORECASE,
)

_META_ARTIFACT_PATTERNS = [
    re.compile(r"^[^<\n]*?(?:в этом дайджесте|итого|в заключение|подведя итог|в итоге|в конечном итоге|итак|в общем|вкратце|как видно|обратите внимание|напомним|также стоит отметить|стоит отметить|следует отметить|важно отметить|подводя итог|резюмируя|ключевые выводы|в целом|как уже упоминалось|напоследок|кратко говоря|среди прочего|собственно говоря|к слову|между прочим|суммируя вышесказанное|суммируя вышеизложенное|в конечном счёте|в конечном счете|очевидно|безусловно|несомненно)[^\n]*\n*", re.IGNORECASE),
