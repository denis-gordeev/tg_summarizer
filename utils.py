...
_META_ARTIFACT_INTRO_ONLY = re.compile(
    r"^[^<\n]*?(?:таким образом|отметим|заметим|в данном обзоре|в данной статье|ниже представлены|ниже приведены|давайте рассмотрим|во-первых|во-вторых|в-третьих|среди прочего|в первую очередь|короче говоря|подытоживая|в частности|перейдём к|перейдем к|возвращаясь к|как показано|как мы видим|как можно заметить|рассмотрим|остановимся на|исходя из вышесказанного|исходя из этого|нужно отметить|необходимо отметить|хочется отметить|хочется подчеркнуть|важно подчеркнуть|не стоит забывать|особо следует|отдельно стоит|в этой связи|подведём итог|подведем итоги|по итогам|по сути|впрочем|помимо прочего|помимо этого|что касается|переходя к|в связи с этим|что важно|стоит понимать|как известно|естественно|строго говоря|честно говоря|откровенно говоря|по существу|в сущности|между тем|тем временем|как правило|ввиду этого|в силу этого|надо сказать|следует понимать|как видим|как можно видеть|отсюда следует|из этого следует|в общем-то|стоит заметить|надо заметить|хочется добавить|стоит добавить|как было сказано|как было отмечено|всё вышесказанное|из вышесказанного|что примечательно|примечательно)[^\n]*\n*",
    re.IGNORECASE,
)

_META_ARTIFACT_PATTERNS = [
    re.compile(r"^[^<\n]*?(?:в этом дайджесте|итого|в заключение|подведя итог|в итоге|в конечном итоге|итак|в общем|вкратце|как видно|обратите внимание|напомним|также стоит отметить|стоит отметить|следует отметить|важно отметить|подводя итог|резюмируя|ключевые выводы|в целом|как уже упоминалось|напоследок|кратко говоря|среди прочего|собственно говоря|к слову|между прочим|суммируя вышесказанное|суммируя вышеизложенное|в конечном счёте|в конечном счете|очевидно|безусловно|несомненно|проще говоря|иными словами|иначе говоря|кстати|кроме того|более того|разумеется|само собой|понятно|одним словом|в общем и целом|к слову сказать|ясное дело|стало быть|по большому счёту|по большому счету|так или иначе|как бы то ни было|в любом случае|что ж|к тому же|другими словами|словом|подводя черту|тем не менее|к сожалению|неудивительно)[^\n]*\n*", re.IGNORECASE),
    ...
    re.compile(r"\n[^<\n]*?другие ссылки:\s*[^\n]*$", re.IGNORECASE),
    re.compile(r"^[^<\n]*?(?:смотри также|подробнее|главное|основное|резюме|сводка|итоги|выводы|результаты)\s*:\s*[^\n]*\n*", re.IGNORECASE),
    re.compile(r"\n[^<\n]*?(?:смотри также|подробнее|главное|основное|резюме|сводка|итоги|выводы|результаты)\s*:\s*[^\n]*$", re.IGNORECASE),
]
