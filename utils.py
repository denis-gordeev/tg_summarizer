...
_META_ARTIFACT_PATTERNS = [
    re.compile(r"^[^<\n]*?(?:в этом дайджесте|итого|в заключение|подведя итог|в итоге|итак|в общем|вкратце|как видно|обратите внимание|напомним|также стоит отметить|стоит отметить|следует отметить|важно отметить|подводя итог|резюмируя)[^\n]*\n*", re.IGNORECASE),
    re.compile(r"\n[^<\n]*?(?:в этом дайджесте|итого|в заключение|подведя итог|в итоге|итак|в общем|вкратце|как видно|обратите внимание|напомним|также стоит отметить|подводя итог|резюмируя)[^\n]*$", re.IGNORECASE),
    re.compile(r"\n[^<\n]*?другие ссылки:\s*[^\n]*$", re.IGNORECASE),
